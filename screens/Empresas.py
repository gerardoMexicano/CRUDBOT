...
class Empresas(tk.Toplevel):
    ...
    def init_widgets(self):
        ...
        tk.Label(self.frame1,text='Agregar y Actualizar datos Empresas',bg='white',fg='black',font=('Kaufmann BT', 13,'bold')).grid(columnspan=2, column=0,row = 0,pady=5)
    ...
    def llenarregistros(self):
        #fill list
        elements= self.basededatos.returtALLempresas()
        for i in elements:
    ...
    def agregar(self):
        ...
        cut=self.cut.get()
        if  nombre!= "" and cut!="":
            self.basededatos.insertempresas(nombre,cut)
    ...
    def actualizar(self):
        ...
        nombre=self.nombre.get()
        cut=self.cut.get()
        self.basededatos.actualizarempresas(id,nombre,cut)
        self.limpiar_campos()
        self.refrescar()

    def borrar(self):
        id=self.obtenerid()
        self.basededatos.deleteempresas(id)
        self.limpiar_campos()
        self.refrescar()
